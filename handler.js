...
╔═════════════════════════╗
  *User Banned*
—◉ *warn ${messageNumber}/3 (Total: 3)*
—◉ ${user.bannedReason ? `\n
*Reason:* ${user.bannedReason}` : '*Reason:* Unspecified'}
—◉ *If you think this is a mistake and you have proof, you can contact the Bot owner to appeal the suspension.*
—◉ *Contact Creator:* wa.me/6281239303460
╚═════════════════════════╝
               `.trim();
              m.reply(messageText);
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
            if (user.commandCount === 2) {
              ...
              if (remainingTime > 0) {
                const messageText = `*wait ${remainingTime} to use the command again*`;
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          this.reply(m.chat, `*Your limit has been used up. Use the command _${usedPrefix}buy <count>_ to buy limit*`, m);
          continue; // Limit habis
        }
        if (plugin.level > _user.level) {
          this.reply(m.chat, `*Your level is too low to use this commands ${plugin.level}, meanwhile your current level ${_user.level}*`, m);
      ...
      if (chat.welcome) {
        ...
        for (const user of participants) {
          ...
          try {
            ...
            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*Description!*') :
            ...
            if (userPrefix && chat.antiArab && botTt.restrict && isBotAdminNn && action === 'add') {
              ...
              this.sendMessage(id, {text: `*[❗] @${user.split('@')[0]} ᴇForeign numbers or rare numbers are NOT allowed in this group, so you will be removed from the group*`, mentions: [user]}, {quoted: fkontak2});
              return;
            }
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await this.reply(nk.from, `Hola *@${nk.from.split('@')[0]}*, The ${nk.isVideo ? 'VideoCall' : 'VoiceCall'} are not allowed, you will be blocked.\n-\nIf you accidentally called contact my creator to unblock you!`, false, {mentions: [nk.from]});
        // let data = global.owner.filter(([id, isCreator]) => id && isCreator)
        // await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Yanz;;;\nFN:Yanz 👑\nORG:Yanz 👑\nTITLE:\nitem1.TEL;waid=6281239303460:+62 8123 9303 460\nitem1.X-ABLabel:Yanz 👑\nX-WA-BIZ-DESCRIPTION: Contact this number for important things.\nX-WA-BIZ-NAME:Yanz 👑\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: 'Yanz 👑', contacts: [{vcard}]}}, {quoted: callmsg});
...
let date = d.toLocaleDateString('id', { day: 'numeric', month: 'long', year: 'numeric' })
 let time = d.toLocaleString('id-ID', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true })
    try {
...
global.dfail = (type, m, conn) => {
  ...
  const prep = conn.sendMessage(m.chat, {
  text: msg,
  contextInfo: {
    externalAdReply: {
